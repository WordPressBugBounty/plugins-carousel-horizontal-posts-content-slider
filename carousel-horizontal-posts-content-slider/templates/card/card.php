...
foreach ($myposts as $wa_post) {
	...
	if ('imageOrVideo'==$qp_post_type) {
		...
		$post_title = $wa_post['label']; //post title 
		$post_link =  $wa_post['slide_url']; //post link
		$post_link_target =  $wa_post['url_target']; //post url target
		$post_content = $wa_post['textarea']; //post content
		$wa_post_id=	$wa_post['attachment']; //post id
		$text_type = $wa_post['textarea']; //post description
	...
	if ( function_exists( 'get_product' ) ) {
		$_product = wc_get_product( $wa_post_id );
	} else {
		...
		$slider_gallery.= '<li style="width:' . $wa_chpcs_query_posts_item_width . 'px; height:' . $wa_chpcs_query_posts_item_height . 'px;" id="wa_chpcs_foo_content' . $id . '" class="wa_chpcs_foo_content wa_chpcs_card">';



	$slider_gallery.= '<div class="item-content">';
		...
		if ($displayimage) {
			...
			//display category
		if ('1'==$wa_chpcs_query_show_categories) {
		...
		if ('1'==$wa_chpcs_query_show_date) {
			...
			$slider_gallery.= '<div class="wa_chpcs_slider_show_date" id="wa_chpcs_slider_show_date' . $id . '">' . get_the_date( '', $wa_post_id ) . '</a></div>';
		...
		// end of list of content div
		$slider_gallery.= '</div>';


		$slider_gallery.= '<div class="clearwidth"></div>';
